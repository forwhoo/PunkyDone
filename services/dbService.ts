...
// ... (Rest of file)
export const logSinglePlay = async (
  track: any,
  listenedMs: number,
  extraData: any = {},
) => {
  // Log
  // if at least 30 seconds (Spotify standard)
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    const { error } = await supabase
      ...
      .upsert(historyItem, {
        onConflict: "spotify_id,played_at",
...
export const syncRecentPlays = async (recentItems: any[], token?: string) => {
  ...
  // This ensures we don't miss items
  // if local state gets out of sync or if we restart the app
  ...
  if (error) {
    ...
    const latest = recentItems.reduce(
      (max, item) =>
    ...
    if (token) {
      ...
      if (artistsNeedingImages.length > 0) {
        console.log(
          `[syncRecentPlays] Fetching images for ${artistsNeedingImages.length} artists`,
        );
        try {
          const { fetchArtistImages } = await import("./spotifyService");
          const artistImages = await fetchArtistImages(
            token,
          ...
          console.error(
            "[syncRecentPlays] Failed to fetch artist images:",
            err,
          );
        }
      }
...
export const fetchListeningStats = async () => {
  ...
  if (data.length > 0) {
    lastEndTime =
      new Date(data[0].played_at).getTime() + (data[0].duration_ms || 0);
  ...
  const currentMins = Math.floor(
    (currentWeekMs % (1000 * 60 * 60)) / (1000 * 60),
  );
  ...
  const totalMs =
    allData?.reduce((acc, item) => acc + (item.duration_ms || 0), 0) || 0;
  const totalMinutes = Math.floor(totalMs / 60000);
  return {
    weeklyTime: `${currentHours}h ${currentMins}
m`,
...
export const fetchDashboardStats = async (
  timeRange: "Daily" | "Weekly" | "Monthly" | "All Time" | "Custom" = "Weekly",
  ...
  if (timeRange === "All Time") {
    ...
    console.log(`[dbService] ⏱️ RPC completed in ${elapsed}
ms`);
    if (error) {
    ...
    console.log(
      `   → ${artistCount} artists, ${songCount} songs, ${albumCount} albums`,
    );
    console.log(
      `   → Total: ${totalMins} Minutes Listened, ${totalTracks} plays (>30s)`,
    );
  ...
  const query = supabase
    .from("listening_history")
    .select(
      "spotify_id, track_name, artist_name, album_name, album_cover, duration_ms, played_at",
    )
    .gte("played_at", startDate.toISOString())
    .order("played_at", { ascending: false });

  // Add end date filter
  // if we have a custom range or not fetching all time
  if (timeRange !== "All Time") {
  ...
  const totalMinutes = Math.round(
    allData.reduce((sum, item) => sum + (item.duration_ms || 0), 0) / 60000,
  );
  ...
  if (timeRange === "Weekly") {
    ...
    allData.forEach((item) => {
      if (item.artist_name && (item.duration_ms || 0) > 30000) {
        currentCounts.set(
          item.artist_name,
          (currentCounts.get(item.artist_name) || 0) + 1,
        );
      }
    });
    ...
    (previousWeekData || []).forEach((item: any) => {
      if (item.artist_name && (item.duration_ms || 0) > 30000) {
        previousCounts.set(
          item.artist_name,
          (previousCounts.get(item.artist_name) || 0) + 1,
        );
      }
    });
  ...
  const artistCounts = new Map<
    string,
    { count: number; time: number; image: string }
  >();
  ...
  const albumStats = new Map<
    string,
    { count: number; duration: number; artist: string; cover: string }
  >();

  // Single pass through data to compute all stats
  for (const item of allData) {
    const durationMs = item.duration_ms || 0;
    const isValidPlay = durationMs > 30000;
    // Only count as play
    // if > 30s
    ...
    if (item.artist_name) {
      if (!artistCounts.has(item.artist_name)) {
        artistCounts.set(item.artist_name, {
          count: 0,
          time: 0,
  ...
  // 4. Hourly Activity Graph (Today) - Use data we already have
  // if today
  ...
  // Bucket by hour (00 to 23) - reuse allData
  // if it contains today
  const hourlyMap: Record<
    number,
    { min: number; count: number; songs: any[] }
  > = {};
  for (let i = 0; i < 24; i++) hourlyMap[i] = { min: 0, count: 0, songs: [] };

  // Filter today's data from allData or fetch separately
  // if needed
  ...
  const recentPlays =
    allData.slice(0, 2000).map((item: any) => ({
      ...item,
      cover: item.album_cover,
    })) || [];
  const elapsed = Math.round(performance.now() - functionStart);
  console.log(`[dbService] ✅ ${timeRange} stats computed in ${elapsed}
ms`);
  console.log(
    `   → ${topArtists.length} artists, ${topSongs.length} songs, ${topAlbums.length} albums`,
  );
  return {
    artists: topArtists,
    songs: topSongs,
...
export const fetchSmartPlaylist = async (concept: { filter: AIFilter }) => {
  try {
    ...
    if (filter.contains) {
      query = query.or(
        `track_name.ilike.%${filter.contains}%,album_name.ilike.%${filter.contains}%,artist_name.ilike.%${filter.contains}%`,
      );
    }
    ...
    if (filter.minDurationMs) {
      filtered = filtered.filter(
        (item) => (item.duration_ms || 0) >= filter.minDurationMs!,
      );
    }
    if (filter.maxDurationMs) {
      filtered = filtered.filter(
        (item) => (item.duration_ms || 0) <= filter.maxDurationMs!,
      );
    }
    ...
    filtered.forEach((item: any) => {
      ...
      if (type === "artist") {
        ...
        // Artist image not available in listening_history, handled in frontend or separate fetch cover = "";
      } else if (type === "album") {
        key = `${album}|||${artist}`;
        title = album;
        cover = item.album_cover;
    ...
    if (sortField === "minutes") {
      results.sort((a, b) =>
        ascending ? a.totalMs - b.totalMs : b.totalMs - a.totalMs,
      );
      ...
      results.sort((a, b) =>
        ascending ? a.plays - b.plays : b.plays - a.plays,
      );
    }
...
export const fetchArtistNetwork = async (limit = 1000) => {
  ...
  for (let i = 0; i < data.length - 1; i++) {
    ...
    for (let j = i + 1; j < Math.min(i + 15, data.length); j++) {
      ...
      if (
        Math.abs(timeA - timeB) < 10 * 60 * 1000 &&
        itemA.artist_name !== itemB.artist_name
      ) {
...
export const uploadExtendedHistory = async (
  ...
  const mapToDbSchema = (item: any): any => {
    // Skip items with no track name (podcasts usually, or errors)
    ...
    return {
      ...
      spotify_id:
        item.spotify_track_uri || item.spotify_episode_uri || "unknown",
      duration_ms: item.ms_played,
      album_cover: null,
      // Will be backfilled after upload
      //       user_timezone: "UTC",
  ...
  try {
    ...
    for (let i = 0; i < total; i += CHUNK_SIZE) {
      ...
      const { error } = await supabase
        ...
        .upsert(chunk, {
          onConflict: "played_at, track_name",
          ignoreDuplicates: true,
        });
      if (error) {
        ...
        return {
          success: false,
          message: `Upload failed at index ${i}: ${error.message}`,
        };
      }
    ...
    console.log(
      "[uploadExtendedHistory] Upload complete, starting image backfill...",
    );
    return {
      success: true,
      message: "Upload complete - backfilling images...",
    };
  } catch (err: any) {
...
export const backfillExtendedHistoryImages = async (
  ...
  try {
    ...
    const { data: statusBefore, error: statusError } =
      await supabase.rpc("count_null_covers");
    if (statusError) {
      console.error(
        "[backfillImages] ❌ count_null_covers not found. Run supabase_backfill_covers.sql first!",
      );
      return {
        success: false,
        message:
          "Please run supabase_backfill_covers.sql in Supabase SQL Editor first!",
      };
    }

    console.log("[backfillImages] 📊 Status before:", statusBefore);
    onProgress(
      `📊 ${statusBefore.null_covers} records need covers (${statusBefore.percent_complete}% complete)`,
    );
    ...
    console.log(
      "[backfillImages] 📚 Calling backfill_covers_from_history RPC...",
    );
    const { data: borrowResult, error: borrowError } = await supabase.rpc(
      "backfill_covers_from_history",
    );
    if (borrowError) {
      ...
      onProgress(
        `📚 Borrowed ${borrowResult.total_updated} covers from listening_history`,
      );
    }

    // STEP 2: Get albums still needing covers (NOW INCLUDES RECORD IDS) onProgress("🎵 Step 2: Finding albums that still need covers..."); console.log("[backfillImages] 🎵 Calling get_albums_needing_covers RPC...");
    const { data: albumsNeeded, error: albumsError } = await supabase.rpc(
      "get_albums_needing_covers",
      { max_results: 2000 },
    );
    if (albumsError) {
      ...
      return {
        success: false,
        message: "Failed to get albums: " + albumsError.message,
      };
    }

    console.log(
      `[backfillImages] 🎵 ${albumsNeeded?.length || 0} unique albums still need covers`,
    );
    ...
    onProgress(
      `🌐 Step 3: Fetching ${albumsNeeded.length} albums from Spotify...`,
    );
    ...
    for (let i = 0; i < albumsNeeded.length; i += chunkSize) {
          ...
          async (item: {
            ...
            try {
              ...
              const query = encodeURIComponent(
                `album:${item.album_name} artist:${item.artist_name}`,
              );
              ...
              console.error(
                `[backfillImages] ❌ Error fetching "${item.album_name}":`,
                e,
              );
            }
          },
      ...
      const percent = Math.round(
        ((i + chunk.length) / albumsNeeded.length) * 100,
      );
      console.log(
        `[backfillImages] 🌐 ${percent}% - Fetched ${fetched}, found ${found}, updated ${totalUpdated}`,
      );
      onProgress(
        `🌐 ${percent}% (found ${found}, saved ${totalUpdated} records)`,
      );
      if (i + chunkSize < albumsNeeded.length) await delay(300);
    }

    console.log(
      `[backfillImages] 📥 Total: fetched ${fetched}, found ${found}, updated ${totalUpdated}`,
    );
...
export const getWrappedStats = async (
  ...
  if (period === "daily") {
    ...
    // Today 12AM label = "Today's Wrapped";
  } else if (period === "weekly") {
    const day = now.getDay();
    const diff = now.getDate() - day + (day === 0 ? -6 : 1);
    // Adjust to Monday startDate = new Date(now.setDate(diff)); startDate.setHours(0, 0, 0, 0); label = "This Week's Wrapped";
  } else {
    startDate = new Date(now.getFullYear(), now.getMonth(), 1);
    // 1st of Month label = "Monthly Wrapped";
  }
  ...
  const songs: Record<
    string,
    { count: number; cover: string; artist: string }
  > = {};
  const albums: Record<
    string,
    { count: number; cover: string; artist: string }
  ...
  rawData.forEach((item) => {
    // Artist
    if (item.artist_name) {
      if (!artists[item.artist_name])
        artists[item.artist_name] = { count: 0, image: item.album_cover || "" };
      artists[item.artist_name].count++;
      if (item.album_cover && !artists[item.artist_name].image)
        artists[item.artist_name].image = item.album_cover;
    }
  ...
  const topArtistEntry = Object.entries(artists).sort(
    (a, b) => b[1].count - a[1].count,
  )[0];
  const topSongEntry = Object.entries(songs).sort(
    (a, b) => b[1].count - a[1].count,
  )[0];
  const topAlbumEntry = Object.entries(albums).sort(
    (a, b) => b[1].count - a[1].count,
  )[0];
  ...
  const albumCovers = [
    ...new Set(rawData.map((item) => item.album_cover).filter(Boolean)),
  ];
  return {
    type: "WRAPPED",
    period: period,
    ...
    topArtist: topArtistEntry
      ? {
          ...
          count: topArtistEntry[1].count,
          image: topArtistEntry[1].image,
        }
      : null,
    topSong: topSongEntry
      ? {
          title: topSongEntry[0],
          count: topSongEntry[1].count,
          cover: topSongEntry[1].cover,
          artist: topSongEntry[1].artist,
        }
      : null,
    topAlbum: topAlbumEntry
      ? {
          title: topAlbumEntry[0],
          count: topAlbumEntry[1].count,
          cover: topAlbumEntry[1].cover,
          artist: topAlbumEntry[1].artist,
        }
      : null,
    topTracks: topTracks,
...
export const getPeakListeningHour = async (
  ...
  const { data: rawData, error } = await supabase
    ...
    .gte("played_at", startDate.toISOString());
  if (error || !rawData || rawData.length === 0) {
    return { hour: 12, label: "Afternoon", count: 0 };
    // Default fallback
  }
...
export const getRadarArtists = async (
  period: "daily" | "weekly" | "monthly" = "daily",
) => {
  const now = new Date();
  let startDate: Date;
  ...
  currentData.forEach((item) => {
    if (item.artist_name) {
      if (!currentArtists[item.artist_name]) {
        currentArtists[item.artist_name] = {
          count: 0,
          image: item.album_cover || "",
        };
      }
  ...
  const previousArtistSet = new Set(
    previousData?.map((d) => d.artist_name) || [],
  );
...
export const getMostSkippedSong = async (
  ...
  const songMap: Record<
    string,
    { totalDuration: number; count: number; artist: string; cover: string }
  > = {};
  data.forEach((item) => {
...
export const getLateNightAnthem = async (
  period: "daily" | "weekly" | "monthly" = "weekly",
) => {
  const now = new Date();
  let startDate: Date;
  ...
  const songMap: Record<
    string,
    { count: number; artist: string; cover: string }
  > = {};
  lateNightPlays.forEach((item) => {
    if (!item.track_name) return;
    if (!songMap[item.track_name])
      songMap[item.track_name] = {
        count: 0,
        artist: item.artist_name,
        cover: item.album_cover || "",
      };
  ...
  const topEntry = Object.entries(songMap).sort(
    (a, b) => b[1].count - a[1].count,
  )[0];
...
export const getRisingStar = async (
  ...
  currentData.forEach((item) => {
    ...
    if (!currentCounts[item.artist_name])
      currentCounts[item.artist_name] = {
        count: 0,
        image: item.album_cover || "",
      };
  ...
  (previousData || []).forEach((item) => {
    if (!item.artist_name) return;
    previousCounts[item.artist_name] =
      (previousCounts[item.artist_name] || 0) + 1;
  });
...
export const getObsessionArtist = async (
  period: "daily" | "weekly" | "monthly" = "weekly",
) => {
  const now = new Date();
  let startDate: Date;
  ...
  const artistSongs: Record<
    string,
    { total: number; image: string; songs: Record<string, number> }
  > = {};
  data.forEach((item) => {
    ...
    artistSongs[item.artist_name].songs[item.track_name] =
      (artistSongs[item.artist_name].songs[item.track_name] || 0) + 1;
  });
  ...
  for (const [artistName, data] of Object.entries(artistSongs)) {
    ...
    const topSongEntry = Object.entries(data.songs).sort(
      (a, b) => b[1] - a[1],
    )[0];
...
export const compareArtistPerformance = async (
  ...
  try {
    ...
    const minutesA = Math.round(
      (dataA?.reduce((sum, item) => sum + (item.duration_ms || 0), 0) || 0) /
        60000,
    );
    const minutesB = Math.round(
      (dataB?.reduce((sum, item) => sum + (item.duration_ms || 0), 0) || 0) /
        60000,
    );
    const deltaPercent =
      playsA > 0
        ? Math.round(((playsB - playsA) / playsA) * 100)
        : playsB > 0
          ? 100
          : 0;
    return {
...
export const getRankShift = async (
  ...
  try {
    ...
    const previousRanking = Object.entries(previousCounts)
      ...
      .map(([name]) => name);
    const currentRank =
      currentRanking.findIndex((name) =>
        name.toLowerCase().includes(entityName.toLowerCase()),
      ) + 1;
    const previousRank =
      previousRanking.findIndex((name) =>
        name.toLowerCase().includes(entityName.toLowerCase()),
      ) + 1;
...
// Get loyalty scorefor an artist within their genre
export const getLoyaltyScore = async (
  artistName: string,
): Promise<{ loyalty_ratio: number; status: string } | null> => {
  try {
...
): Promise<{ top_entities: Array<{ name: string; share: string }> } | null> => {
  try {
    const field = entityType === "artist" ? "artist_name" : "album_name";
    // Using album as genre proxy
    const { data } = await supabase.from("listening_history").select(field);
...
export const getDiscoveryDate = async (
  artistName: string,
): Promise<{ first_played: string; first_song: string } | null> => {
  try {
...
export const getBingeSessions = async (
  thresholdMinutes: number = 60,
): Promise<{
  sessions: Array<{ date: string; duration: string; artist: string }>;
} | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (
        ...
        (i > 0 &&
          playTime.getTime() -
            (new Date(data[i - 1].played_at).getTime() +
              (data[i - 1].duration_ms || 0)) >
            gapThreshold)
      ) {
        ...
        if (
          currentSession &&
          currentSession.totalMs >= thresholdMinutes * 60 * 1000
        ) {
          sessions.push({
    ...
    if (
      ...
      sessions.push({
        date: currentSession.start.toLocaleDateString(),
        duration: `${Math.round(currentSession.totalMs / 60000)}
m`,
        artist: currentSession.artist,
      });
    }
...
export const getOneHitWonders = async (
  minPlays: number = 5,
): Promise<{
  artists: Array<{ name: string; top_song: string; song_share: string }>;
} | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
    ...
    data.forEach((item) => {
      ...
      artistSongs[item.artist_name][item.track_name] =
        (artistSongs[item.artist_name][item.track_name] || 0) + 1;
    });
    const oneHitWonders: Array<{
      name: string;
      top_song: string;
      song_share: string;
    }> = [];
    ...
    for (const [artist, songs] of Object.entries(artistSongs)) {
      const totalPlays = Object.values(songs).reduce(
        (sum, count) => sum + count,
        0,
      );
...
export const getAlbumCompletionist = async (
  albumName: string,
): Promise<{
...
export const getEarwormReport = async (
  daysBack: number = 7,
): Promise<{
  song: string;
  repeat_count: number;
  max_repeats_in_one_hour: number;
} | null> => {
  try {
    ...
    const maxHourlyRepeats = Math.max(
      ...Object.values(hourlyRepeats[topSong[0]] || {}),
    );
    return {
      song: topSong[0],
...
export const getWorkVsPlayStats = async (): Promise<{
  workday_top: string[];
  weekend_top: string[];
} | null> => {
  try {
...
// Get seasonal vibe
export const getSeasonalVibe = async (
  season: "Summer" | "Winter" | "Spring" | "Fall",
): Promise<{ top_genres: string[]; avg_tempo: string } | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
    ...
    return {
      ...
      avg_tempo: "Medium",
      // Would need Spotify audio features
    };
  } catch (error) {
...
export const getAnniversaryFlashback = async (
  date?: string,
): Promise<{
  date: string;
  top_song: string;
...
export const getCommuteSoundtrack = async (): Promise<{
  top_artists: string[];
  time_window: string;
} | null> => {
  try {
...
// Get sleep pattern
export const getSleepPattern = async (): Promise<{
  avg_stop_time: string;
  sleep_genres: string[];
} | null> => {
  try {
    const { data } = await supabase
    ...
    const avgStopHour =
      stopHours.length > 0
        ? Math.round(
            stopHours.reduce((sum, h) => sum + h, 0) / stopHours.length,
          )
        : 23;
    const avgStopTime =
      avgStopHour === 0
    ...
    nightListening.forEach((item) => {
      if (item.album)
        genreCounts[item.album] = (genreCounts[item.album] || 0) + 1;
    });
...
// Get diversity index
export const getDiversityIndex = async (
  ...
  try {
    ...
    const uniqueArtists = new Set(
      data.map((item) => item.artist_name).filter(Boolean),
    );
    const totalPlays = data.length;
    ...
    const diversityScore = Math.min(
      100,
      Math.round((uniqueCount / totalPlays) * 100 * 10),
    );
    const repeatRate =
      totalPlays > 0 ? (totalPlays - uniqueCount) / totalPlays : 0;
    const repeatLabel =
      repeatRate < 0.3 ? "low" : repeatRate < 0.6 ? "medium" : "high";
    return {
...
export const getGenreEvolution = async (
  months: number = 6,
): Promise<{ timeline: Array<{ month: string; top: string }> } | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
      .select("artist_name, album_name, played_at")
    ...
    for (let i = months - 1; i >= 0; i--) {
      ...
      monthData.forEach((item) => {
        ...
        if (item.artist_name) {
          genreCounts[item.artist_name] =
            (genreCounts[item.artist_name] || 0) + 1;
        }
      });
...
export const getSkipRateByArtist = async (
  artistName: string,
): Promise<{ skip_percent: string; avg_listen_duration: string } | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
      .select("duration_ms")
...
export const getGatewayTracks = async (
  artistName: string,
): Promise<{ gateway_song: string; follow_up_plays: number } | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
...
export const getTopCollaborations = async (): Promise<{
  pairs: Array<{ artists: string; co_occurrence: number }>;
} | null> => {
  try {
    const { data } = await supabase
      .from("listening_history")
    ...
    data.forEach((item) => {
      ...
      if (
        !lastTime ||
        currentTime.getTime() - lastTime.getTime() > 30 * 60 * 1000
      ) {
...
export const getMilestoneTracker = async (
  targetPlays: number,
  artistName?: string,
): Promise<{
  artist: string;
      ...
      data.forEach((item) => {
        if (item.artist_name) {
          artistCounts[item.artist_name] =
            (artistCounts[item.artist_name] || 0) + 1;
        }
      });
...
export const getForgottenFavorites = async (
  minPlays: number = 5,
): Promise<{
  forgotten_songs: Array<{
    title: string;
    artist: string;
    cover: string;
    plays: number;
  ...
  try {
    ...
    const songStats: Record<
      string,
      { plays: number; lastPlayed: Date; artist: string; cover: string }
    > = {};
    data.forEach((item) => {
      if (!item.track_name) return;
      const date = new Date(item.played_at);
      if (!songStats[item.track_name]) {
        songStats[item.track_name] = {
          plays: 0,
          lastPlayed: date,
    ...
    const forgotten = Object.entries(songStats)
      .filter(
        ([_, stats]) =>
          stats.plays >= minPlays && stats.lastPlayed < thirtyDaysAgo,
      )
...
export const getObsessionScore = async (
  ...
  try {
    ...
    // Find artist with highest obsession scorelet bestArtist = null;
    let bestScore = 0;
    ...
    for (const [artist, data] of Object.entries(artistSongs)) {
      ...
      const obsessionScore = Math.round(
        dominancePercent * (1 + Math.log10(data.total)),
      );
