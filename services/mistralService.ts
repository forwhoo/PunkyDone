...
export const TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        title: {
          type: "string",
          description: "A short name for the skill (e.g. 'Data Scientist')",
        },
        description: {
          type: "string",
        ...
        system_prompt: {
          type: "string",
          description:
            "The full instruction the AI will follow when this skill is active",
        },
      },
  ...
  },
  {
    ...
    description:
      "Calculate what percentage of listening time a specific artist, song, or album takes up.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Get the artist with the biggest increase in plays compared to previous period.",
    parameters: {
      type: "object",
      properties: {
        period: { type: "string", enum: ["daily", "weekly", "monthly"] },
  ...
  },
  {
    ...
    description:
      "Get the song the user plays most during late night hours (1AM-5AM).",
    parameters: {
      type: "object",
      properties: {
        period: { type: "string", enum: ["daily", "weekly", "monthly"] },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        field: {
          type: "string",
          enum: ["artist_name", "album_name", "track_name"],
        },
        value: { type: "string" },
        contains: { type: "string" },
        time_of_day: {
          type: "string",
          enum: ["morning", "afternoon", "evening", "night", "latenight"],
        },
        day_of_week: { type: "string", enum: ["weekday", "weekend"] },
        recent_days: { type: "number" },
        sort_by: {
          type: "string",
          enum: ["plays", "minutes", "recency", "duration"],
        },
  ...
  },
  {
    ...
    description:
      "Returns the ratio of this artist's plays vs. all other artists.",
    parameters: {
      type: "object",
      properties: {
        artist_name: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Returns a breakdown of what percentage of the 'total pie' an entity owns.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Finds the exact timestamp and track of the user's very first interaction with an artist.",
    parameters: {
      type: "object",
      properties: {
        artist_name: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Identifies 'binge' events where the user listened to one artist for a long continuous block.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Finds artists where the user loves exactly ONE song but never listens to the rest.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Finds the song the user has 'looped' the most in a short window.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Analyzes if the user's music taste changes based on the season.",
    parameters: {
      type: "object",
      properties: {
        season: {
  ...
  },
  {
    ...
    description:
      "Returns what the user was listening to exactly one year ago today.",
    parameters: {
      type: "object",
      properties: {
        date: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Detects when the user stops listening at night and what 'sleep' music they use.",
    parameters: { type: "object", properties: {}, required: [] },
  },
  {
  ...
  },
  {
    ...
    description:
      "Shows how the user's favorite genre has changed month-over-month.",
    parameters: {
      type: "object",
      properties: {
        months: { type: "number" },
  ...
  },
  {
    ...
    description:
      "Specifically checks if a user skips an artist's songs more than others.",
    parameters: {
      type: "object",
      properties: {
        artist_name: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Finds which artists are most frequently played in the same listening session.",
    parameters: { type: "object", properties: {}, required: [] },
  },
  {
  ...
  },
  {
    name: "vote",
    description:
      "Create a poll or multi-select for the user to help decide or provide more information.",
    parameters: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Finds songs the user used to listen to heavily but hasn't played in the last 30 days.",
    parameters: {
      type: "object",
      properties: {
        min_plays: {
          type: "number",
          description:
            "Minimum number of plays to be considered a favorite (default 5)",
        },
      },
      required: [],
    },
  },
  {
    name: "set_skill",
    description:
      "Set the AI skill for future interactions. The skill should be one of: 'Music Critic', 'Stan', 'Data Scientist', 'Roaster', or 'Default'.",
    parameters: {
      type: "object",
      properties: {
        skill: {
    ...
    switch (funcName) {
      ...
      case "get_top_songs": {
        ...
        if (funcArgs.start_date && funcArgs.end_date) {
          stats = await fetchDashboardStats("Custom", {
            start: funcArgs.start_date,
            end: funcArgs.end_date,
          });
        } else {
        ...
        return {
          period,
          custom_range: funcArgs.start_date
            ? `${funcArgs.start_date} to ${funcArgs.end_date}`
            : null,
          count: songs.length,
      ...
      case "get_listening_time": {
        ...
        const totalMinutes = (dashboard.songs || []).reduce(
          (acc: number, s: any) => {
            const mins = s.timeStr
              ? parseInt(s.timeStr.replace(/[^0-9]/g, ""), 10)
              : 0;
            return acc + mins;
          },
          0,
        ...
        return {
          ...
          longestSession:
            listeningStats.extraStats?.longestSessionHours || "Unknown",
        };
      }
      case "get_obsession_orbit": {
        ...
        if (funcArgs.artist_name) {
          ...
          return {
            ...
            is_obsession:
              obsession?.artist?.toLowerCase().includes(target) || false,
      ...
      case "get_artist_streak": {
        ...
        const rank = found ? artists.indexOf(found) + 1 : -1;
        const artistSongs = (stats.songs || []).filter((s: any) =>
          (s.artist || "").toLowerCase().includes(target),
        );
        return {
          ...
          top_song: artistSongs[0]
            ? {
                title: artistSongs[0].title,
                plays: artistSongs[0].listens || 0,
              }
            : null,
        };
      }
      case "get_listening_percentage": {
        ...
        if (entityType === "artist") {
          ...
          artists.forEach((a: any) => {
            const mins = a.timeStr
              ? parseInt(a.timeStr.replace(/[^0-9]/g, ""), 10)
              : 0;
            totalMinutes += mins;
            if ((a.name || "").toLowerCase().includes(entityName))
              entityMinutes += mins;
          });
          ...
          songs.forEach((s: any) => {
            const mins = s.timeStr
              ...
              : 0;
            totalMinutes += mins;
            if ((s.title || "").toLowerCase().includes(entityName))
              entityMinutes += mins;
          });
        } else {
          ...
          albums.forEach((a: any) => {
            const mins = a.timeStr
              ...
              : 0;
            totalMinutes += mins;
            if ((a.title || "").toLowerCase().includes(entityName))
              entityMinutes += mins;
          });
        }
        const percentage =
      ...
      case "get_late_night_anthem": {
        ...
        return (
          anthem || { title: "No late night plays found", artist: "", plays: 0 }
        );
      }
      ...
      case "fetch_image": {
        if (!token)
          return { image_url: null, error: "No Spotify token available" };
      ...
      case "get_heatmap_data": {
        ...
        const filtered = rawData.filter(
          (d: any) => new Date(d.played_at) >= cutoff,
        );
        ...
        return {
          ...
          peak_hour: peakHour
            ? {
                ...
                label: `${Number(peakHour[0]) % 12 || 12}${Number(peakHour[0]) < 12 ? "AM" : "PM"}`,
                plays: peakHour[1],
              }
            : null,
          peak_day: peakDay
            ? { day: dayNames[Number(peakDay[0])], plays: peakDay[1] }
            : null,
        };
      }
      case "get_artist_network": {
        ...
        for (const [a, connections] of Object.entries(network.pairs)) {
          for (const [b, count] of Object.entries(
        ...
        return {
          top_artists: artistList.map((a: any) => ({
            name: a.name,
            plays: a.count,
          })),
      ...
      case "get_genre_breakdown": {
        ...
        artists.forEach((a: any) => {
          ...
          genres.forEach(
            (g: string) => (genreCounts[g] = (genreCounts[g] || 0) + weight),
          );
        });
        const totalWeight = Object.values(genreCounts).reduce(
          (a, b) => a + b,
          0,
        );
      ...
      case "get_recent_plays": {
        ...
        const recentPlays = (stats.recentPlays || stats.songs || []).slice(
          0,
          limit,
        );
        return {
      ...
      case "compare_periods": {
        ...
        const totalMinsA = (statsA.songs || []).reduce(
          (acc: number, s: any) =>
            acc +
            (s.timeStr ? parseInt(s.timeStr.replace(/[^0-9]/g, ""), 10) : 0),
          0,
        );
        const totalMinsB = (statsB.songs || []).reduce(
          (acc: number, s: any) =>
            acc +
            (s.timeStr ? parseInt(s.timeStr.replace(/[^0-9]/g, ""), 10) : 0),
          0,
        ...
        return {
          ...
          comparison: {
            ...
            songs_diff:
              (statsA.songs || []).length - (statsB.songs || []).length,
            artists_diff:
              (statsA.artists || []).length - (statsB.artists || []).length,
          },
        };
      }
      case "get_album_covers": {
        ...
        try {
          ...
          return {
            ...
            covers: Object.entries(images).map(([name, url]) => ({
              name,
              image_url: url,
            })),
          };
      ...
      case "compare_artist_performance": {
        return (
          (await compareArtistPerformance(
      ...
      case "get_loyalty_score": {
        return (
          (await getLoyaltyScore(funcArgs.artist_name)) || {
            error: "No loyalty data found",
          }
        );
      }
      case "get_market_share": {
        return (
          (await getMarketShare(funcArgs.entity_type)) || {
      ...
      case "get_discovery_date": {
        return (
          (await getDiscoveryDate(funcArgs.artist_name)) || {
            error: "No discovery date found",
          }
        );
      }
      case "get_binge_sessions": {
        return (
          (await getBingeSessions(funcArgs.threshold_minutes || 60)) || {
            error: "No binge sessions found",
          }
        );
      }
      case "get_one_hit_wonders": {
        return (
          (await getOneHitWonders(funcArgs.min_plays || 5)) || {
            error: "No one-hit wonders found",
          }
        );
      }
      case "get_album_completionist": {
        return (
          (await getAlbumCompletionist(funcArgs.album_name)) || {
            error: "No album completion data found",
          }
        );
      }
      case "get_earworm_report": {
        return (
          (await getEarwormReport(funcArgs.days_back || 7)) || {
            error: "No earworm data found",
          }
        );
      }
      case "get_work_vs_play_stats": {
        return (
          (await getWorkVsPlayStats()) || {
            error: "No work vs play data available",
          }
        );
      }
      case "get_seasonal_vibe": {
        return (
          (await getSeasonalVibe(funcArgs.season)) || {
      ...
      case "get_anniversary_flashback": {
        return (
          (await getAnniversaryFlashback(funcArgs.date)) || {
            error: "No anniversary data found",
          }
        );
      }
      case "get_commute_soundtrack": {
        return (
          (await getCommuteSoundtrack()) || {
            error: "No commute data available",
          }
        );
      }
      case "get_sleep_pattern": {
        return (
          (await getSleepPattern()) || { error: "No sleep pattern data found" }
        );
      }
      case "get_diversity_index": {
        return (
          (await getDiversityIndex(funcArgs.time_range || "monthly")) || {
            error: "No diversity data available",
          }
        );
      }
      case "get_genre_evolution": {
        return (
          (await getGenreEvolution(funcArgs.months || 6)) || {
            error: "No genre evolution data found",
          }
        );
      }
      case "get_skip_rate_by_artist": {
        return (
          (await getSkipRateByArtist(funcArgs.artist_name)) || {
            error: "No skip rate data found",
          }
        );
      }
      case "get_gateway_tracks": {
        return (
          (await getGatewayTracks(funcArgs.artist_name)) || {
            error: "No gateway tracks found",
          }
        );
      }
      case "get_top_collaborations": {
        return (
          (await getTopCollaborations()) || {
            error: "No collaboration data available",
          }
        );
      }
      case "get_milestone_tracker": {
        return (
          (await getMilestoneTracker(
            funcArgs.target_plays,
            funcArgs.artist_name,
          )) || { error: "No milestone tracking data available" }
        );
      }
      case "get_forgotten_favorites": {
        const { getForgottenFavorites } = await import("./dbService");
        return (
          (await getForgottenFavorites(funcArgs.min_plays || 5)) || {
            error: "No forgotten favorites found",
          }
        );
      }
      case "get_obsession_score": {
        return (
          (await getObsessionScore(
            ...
            funcArgs.start_date,
            funcArgs.end_date,
          )) || { error: "No obsession score data found" }
        );
      }
      case "vote": {
        return {
      ...
      case "set_skill": {
        return { status: "skill_set", skill: funcArgs.skill };
      }
      case "create_skill": {
        return {
...
// ─── AGENT SYSTEM PROMPT ─────────────────────────────────────────
const AGENT_SYSTEM_PROMPT = `You are **Claudius**, the AI music analytics agent.
...
export const streamMusicQuestionWithTools = async (
  ...
  console.log("[agentTools] streamMusicQuestionWithTools called", {
    question,
    modelId,
    webSearchEnabled,
  ...
  try {
    ...
    if (!client) {
      onChunk({
        type: "text",
        content: "Configure VITE_MISTRAL_API_KEY to use chat features.",
      });
      return;
    }
    ...
    const messages: any[] = [
      {
        role: "system",
        content: AGENT_SYSTEM_PROMPT + skillInstruction,
      },
    ];
    ...
    if (history.length > 0) {
      ...
      messages.push({
        role: "assistant",
        content: "I understand. I am Claudius, ready to analyze music data.",
      });
    }
    ...
    while (continueLoop) {
      ...
      for await (const chunk of stream) {
        ...
        if ((delta as any).thinking) {
          onChunk({
            type: "thinking",
            content: (delta as any).thinking as string,
          });
        }
...
export const answerMusicQuestionWithTools = async (
  ...
  console.log("[agentTools] answerMusicQuestionWithTools called", {
    question,
    modelId,
    webSearchEnabled,
    historyLength: history.length,
...
export const generateMusicInsights = async (
  contextData: string,
): Promise<string> => {
  try {
    const client = getAiClient();
...
export const answerMusicQuestion = async (
  question: string,
  context: any,
): Promise<string> => {
  try {
...
export const generateMusicInsight = async (
  query: string,
  stats: any,
): Promise<string> => {
  try {
...
export const generateRankingInsights = async (
  items: string[],
...
export const generateWeeklyInsightStory = async (
  context: any,
): Promise<any[]> => {
  try {
    const client = getAiClient();
...
export const generateDynamicCategoryQuery = async (
  context: any,
  userPrompt?: string,
): Promise<any[]> => {
  try {
...
export const generateWrappedStory = async (period: string): Promise<any> => {
  try {
    ...
    const result = await client.chat.complete({
      model: DEFAULT_MODEL_ID,
      messages: [
        { role: "user", content: `Generate wrapped story JSON for ${period}` },
      ],
      responseFormat: { type: "json_object" },
    });
    const content = result.choices?.[0]?.message?.content;
  ...
  try {
    ...
    const result = await client.chat.complete({
      ...
      messages: [
        {
          role: "user",
          content: `Generate vibe check JSON for tracks: ${tracks.map((t) => t.title).join(",")}`,
        },
      ],
      responseFormat: { type: "json_object" },
    });
  ...
  try {
    ...
    const result = await client.chat.complete({
      ...
      messages: [
        {
          role: "user",
          content: `Generate music quiz JSON from stats: ${JSON.stringify(stats)}`,
        },
      ],
      responseFormat: { type: "json_object" },
    });
  ...
  try {
    ...
    const result = await client.chat.complete({
      ...
      messages: [
        {
          role: "user",
          content: `Generate fruit vibe JSON for tracks: ${tracks.map((t) => t.title).join(",")}`,
        },
      ],
      responseFormat: { type: "json_object" },
    });
...
export const generateTopAlbumFunFact = async (album: any): Promise<string> => {
  try {
    ...
    const result = await client.chat.complete({
      model: DEFAULT_MODEL_ID,
      messages: [
        { role: "user", content: `Fun fact about album ${album.title}` },
      ],
    });
    return (result.choices?.[0]?.message?.content as string) || "";
  } catch (e) {
