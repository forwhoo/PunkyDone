    ...
    img.onload = () => {
      try {
        ...
        for (let i = 0; i < data.length; i += 16) {
          ...
          if (
            brightness > MIN_PIXEL_BRIGHTNESS &&
            brightness < MAX_PIXEL_BRIGHTNESS
          ) {
            r += pr;
            g += pg;
            b += pb;
        ...
        //
        //
        // if color is too desaturated for a visible aura
        const max = Math.max(r, g, b),
          min = Math.min(r, g, b);
        if (max - min < MIN_SATURATION_RANGE) {
          resolve(FALLBACK_AURA_COLOR);
          return;
        }
...
const MobileArtistCard = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className="relative w-[210px] h-[280px] shrink-0 snap-start rounded-[28px] overflow-hidden border border-white/[0.08] active:scale-[0.97] transition-transform"
  >
    {" "}
    <img
      src={
...
function App() {
  ...
  const [artistDiscoveryDate, setArtistDiscoveryDate] = useState<string | null>(
    null,
  );
    ...
    const loadImages = async () => {
      ...
      const topArtists =
        dbUnifiedData?.artists?.length > 0
          ? dbUnifiedData.artists
          : data?.artists || [];
      topArtists
        .slice(0, 15)
        .forEach((a: Artist) => artistsToFetch.add(a.name));
...
`,
  ...
  const refreshDbStats = useCallback(async () => {
    ...
    const currentCustomRange = customDateRange;

    // Capture custom range
    logger.info(
      `[App] 📊 Refreshing DB Stats for ${requestedRange}... (fetchId: ${currentFetchId})`,
    );
    try {
      const stats = await fetchListeningStats();
      const dashboardStuff =
        requestedRange === "Custom" && currentCustomRange
      ...
      //
      //
      // if this request is still the latest one
      if (currentFetchId !== fetchIdRef.current) {
        logger.info(`[App] ⚠️ Discarding stale response for ${requestedRange}(fetchId: ${currentFetchId}
, current: ${fetchIdRef.current}
      ...
      const currentCharts = await fetchCharts(
        requestedRange.toLowerCase() as any,
      );
      logger.info(
        `[App] ✅ Dashboard Stats for ${requestedRange}
:`,
        {
          hasStats: !!stats,
  ...
  // Update the ref whenever
  // refreshDbStats changes
  useEffect(() => {
  ...
  const safeAlbums =
    dbUnifiedData?.albums?.length > 0
      ? dbUnifiedData.albums
      : data?.albums || [];
  const safeSongs =
    dbUnifiedData?.songs?.length > 0 ? dbUnifiedData.songs : data?.songs || [];
  const safeRecent =
    dbUnifiedData?.recentPlays?.length > 0
      ? dbUnifiedData.recentPlays
      : data?.recentRaw || [];
  const hasDbData =
    safeArtists.length > 0 || safeSongs.length > 0 || safeAlbums.length > 0;
  ...
  const selectedArtistStats = useMemo(() => {
    ...
    const artistPlays = safeRecent.filter(
      (play: any) =>
        play.artist_name === artistName || play.artist === artistName,
    );
    const totalPlaysAllArtists = safeArtists.reduce(
      (sum: number, artist: Artist) => sum + (artist.totalListens || 0),
      0,
    );
    const popularityScore =
      totalPlaysAllArtists > 0
        ? Math.round(
            ((selectedTopArtist.totalListens || 0) / totalPlaysAllArtists) *
              100,
          )
        : 0;
    ...
    const dailyAverage =
      activeDays > 0
        ? Math.round((selectedTopArtist.totalListens || 0) / activeDays)
        : 0;
    const uniqueSongs = new Set(
      artistPlays.map((p: any) => p.track_name || p.title),
    );
      ...
      <div className="fixed inset-0 bg-background text-foreground flex flex-col items-center justify-between p-6 overflow-hidden font-body">
        ...
        <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none opacity-50">
          ...
          <Particles
            ...
            particleSpread={12}
            speed={0.12}
            particleColors={["#252523", "#a0a0a0", "#404040"]}
        ...
        <div className="relative z-10 w-full max-w-sm flex flex-col items-center gap-8 pb-10 animate-fade-in">
          ...
          <div className="w-full space-y-3">
            ...
            <button
              ...
              disabled={connecting}
              className="auth-button-primary flex items-center justify-center gap-2 shadow-xl hover: active:scale-[0.98] transition-all"
            >
              {" "}
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar snap-x px-1">
                    ...
                    {safeAlbums
                      ...
                      .map((album: Album, index: number) => (
                        <div
                          key={album.id}
                          className="w-[180px] shrink-0 snap-start group cursor-pointer"
