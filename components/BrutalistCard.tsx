...
export const BrutalistCard: React.FC<BrutalistCardProps> = ({
  ...
  const hp = Math.min(
    999,
    Math.round(
      (parseInt(artist.timeStr?.replace(/[^0-9]/g, "") || "0") / 60) * 10,
    ),
  );
  // Minutes * 10 capped
  ...
  return (
    <div className="relative group perspective-1000">
      ...
      <motion.div
        ref={cardRef}
        className={` relative w-64 h-96 bg-[#E0E0E0] border-[6px] border-black   flex flex-col overflow-hidden transition-transform duration-300 preserve-3d ${isHolo ? "hover:rotate-y-12 hover:rotate-x-12" : ""} `}
      ...
      >
        ...
        <div className="bg-[#CCFF00] border-b-[4px] border-black p-2 flex justify-between items-center relative z-10">
          ...
          <span className="font-bold text-xs flex items-center gap-1">
            {" "}
            HP{" "}
            <span className="text-lg leading-none text-red-600 font-black">
              {hp}
            </span>{" "}
          </span>{" "}
        </div>{" "}
        ...
        <div className="relative mx-3 mt-3 border-[4px] border-black aspect-square bg-card overflow-hidden">
          ...
          {/* Holo Shine */}{" "}
          <div
            className={`absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent -translate-x-full group-hover:animate-shine pointer-events-none`}
          />{" "}
        </div>{" "}
        {/* Info Bar */}{" "}
        ...
        <div className="p-3 flex-1 flex flex-col justify-between bg-[#F0F0F0]">
          ...
          <div className="flex justify-between items-end border-b-2 border-black/10 pb-2">
            ...
            <div className="flex flex-col text-right">
              ...
              <span className="font-bold text-xs uppercase truncate max-w-[100px]">
                Sonic Boom
              </span>{" "}
            </div>{" "}
          </div>{" "}
          <div className="flex justify-between items-center mt-2">
            {" "}
            <h3
              className="font-black text-lg leading-none uppercase truncate max-w-[140px]"
              title={artist.name}
            >
              {" "}
              {artist.name}{" "}
