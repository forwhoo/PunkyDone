...
export const TopCharts: React.FC<TopChartsProps> = ({
  ...
  React.useEffect(() => {
    if (activeTab === "Songs") {
      // Only fetch
      // if "Songs" because that's what the SQL supports right now
      const period = timeRange.toLowerCase() as
        | "daily"
        | "weekly"
        | "monthly"
        | "all time";
  ...
  // Merge dynamic chart data with passed-in songs
  //
  // if needed,

  // OR just use chartData
  // if available and activeTab is 'Songs'.

  // For now, let's prefer chartData for Songs tab
  // if it has content
  ...
  const getListData = () => {
    switch (activeTab) {
      ...
      case "Albums":
        return albums;
      case "Songs":
        return chartData.length > 0 ? chartData : songs;
      default:
        return [];
    }
  };
  ...
  const handleImageError = (
    ...
    e.currentTarget.src =
      "https://ui-avatars.com/api/?background=2C2C2E&color=fff&size=128&bold=true";
  };
  return (
    <div className="flex flex-col overflow-visible relative">
      ...
      <div className="flex-1 overflow-hidden">
        ...
        <div className="w-full">
          <div className="grid grid-cols-[40px_1fr_50px_50px_50px] px-3 py-3 border-b border-border text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
          ...
          <div className="flex flex-col max-h-[400px] overflow-y-auto custom-scrollbar">
            {listData.slice(0, 10).map((item: any, idx) => {
              ...
              // if not present (remove in prod if always present)
              const trend = item.trend || "NEW";
              const prev =
                item.prev ||
                (trend === "NEW" ? "-" : rank + Math.floor(Math.random() * 5));
              ...
              return (
                <div
                  key={item.id}
                  className="grid grid-cols-[40px_1fr_50px_50px_50px] items-center px-3 py-3 hover:bg-secondary/50 transition-colors group border-b border-border last:border-0"
                >
                  {/* Rank + Trend Column */}
                  <div className="flex flex-col items-center justify-center">
                    <span
                      className={`text-sm font-black ${rank <= 3 ? "text-[#d97757]" : "text-foreground"}`}
                    >
                      {rank}
                    </span>
                    <div className="flex items-center gap-0.5 mt-0.5">
                      {trend === "NEW" ? (
                        <span className="text-[8px] font-black text-[#d97757] uppercase">
                          NEW
                        </span>
                      ) : (
                        getTrendIcon(trend)
                  ...
                  <div className="flex items-center gap-3 min-w-0 pr-2">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-[#2C2C2E] relative shrink-0 border border-border">
                      <img
                      ...
                      />
                      {activeTab === "Songs" && (
                        <div className="absolute inset-0 bg-card/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                          <Play className="w-4 h-4 text-foreground fill-white" />
                        </div>
                      )}
                    </div>
                    <div className="min-w-0">
                      <div className="font-semibold text-foreground text-sm truncate group-hover:text-[#d97757] transition-colors">
