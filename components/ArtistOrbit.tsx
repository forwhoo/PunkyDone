    ...
    return (
        <div className="relative w-full aspect-square max-w-[500px] mx-auto flex items-center justify-center p-4">
            ...
            <motion.div
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="relative z-10 w-24 h-24 sm:w-32 sm:h-32 rounded-full overflow-hidden border-4 border-white/10 shadow-2xl"
            ...
            >
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent flex items-end justify-center pb-2">
                    <span className="text-[10px] font-bold text-white uppercase tracking-tighter truncate px-2">{centralNode.name}</span>
                </div>
            </motion.div>
            ...
            {nodes.map((node, idx) => {
                ...
                return (
                    <motion.div
                        ...
                        className="absolute w-12 h-12 sm:w-16 sm:h-16 rounded-xl overflow-hidden border border-white/10 shadow-xl group cursor-pointer"
                    >
                        <img src={node.image} alt={node.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform" />
                        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-center p-1">
                            <span className="text-[8px] font-bold text-white truncate w-full">{node.name}</span>
                            <div className="flex items-center gap-1 mt-0.5">
                                <Disc size={8} className="text-white/70" />
                                <span className="text-[8px] text-white/90">{node.plays}</span>
                            </div>
                        </div>
                    </motion.div>
                );
