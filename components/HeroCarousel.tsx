...
const SUGGESTIONS = [
  {
    label: "Analyze my top genres",
    icon: Music,
    color: "from-pink-500 to-rose-500",
  },
  {
    label: "Predict my next favorite",
    icon: Sparkles,
    color: "from-purple-500 to-indigo-500",
  },
  {
    label: "Morning drive vibe",
    icon: Headphones,
    color: "from-orange-400 to-amber-400",
  },
  {
    label: "Workout energy check",
    icon: Clock,
...
export const HeroCarousel = ({
      ...
      ? todayPlays
          .reduce((acc: any, curr: any) => {
            const existing = acc.find(
              (item: any) => item.track_name === curr.track_name,
            );
  ...
  const peakHourFormatted = peakHour
    ? `${peakHour[0] % 12 || 12}${parseInt(peakHour[0]) >= 12 ? "PM" : "AM"}`
    : "N/A";
