...
export const ActivityHeatmap: React.FC<ActivityHeatmapProps> = ({
  ...
  const { dailyData, dailyTracks, maxCount, totalPlays, totalMinutes } =
    useMemo(() => {
      ...
      return {
        dailyData: grouped,
        dailyTracks: tracks,
        maxCount: Math.max(max, 5),
        totalPlays: tPlays,
        totalMinutes: Math.round(tMins),
      };
    }, [filteredHistory]);
  const weeks = useMemo(() => {
  ...
  return (
    <div
      className={`relative flex flex-col ${isCompact ? "gap-4" : "lg:flex-row gap-8"} items-start transition-all duration-500 overflow-x-hidden w-full`}
    >
      {/* LEFT: Heatmap Container */}
      <div
        className={`w-full transition-all duration-500 ease-in-out ${!isCompact && selectedDate ? "lg:w-[calc(100%-390px)]" : "w-full"}`}
      >
        <div
          className={`w-full max-w-4xl mx-auto ${isCompact ? "mb-4" : "mb-8"} animate-in fade-in slide-in-from-top-4 duration-700`}
        >
            ...
            <Popover open={dropdownOpen} onOpenChange={setDropdownOpen}>
              <PopoverTrigger asChild>
                <button className="flex items-center gap-1.5 text-[12px] font-bold text-foreground bg-secondary/50 px-4 py-2 rounded-xl border border-border hover:bg-secondary active:scale-95 transition-all shadow-sm">
                  ...
                  <ChevronDown
                    size={14}
                    className={`transition-transform duration-300 ml-1 opacity-50 ${dropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
              </PopoverTrigger>
              <PopoverContent
                  ...
                  {monthData.days.map((day, dIdx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {selectedDate === day.date || intensity > 0.75 ? (
