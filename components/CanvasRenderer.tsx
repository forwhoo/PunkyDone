  ...
  useEffect(() => {
    const loadBabelAndTranspile = async () => {
      ...
      try {
        ...
        // We wrap the code to
        //
        // return a default export or a specific component
        ...
        // Create a
        // function that executes the code in a sandbox with our scope
        const scope = {
          React,
          useState,
          useEffect,
        ...
        if (
          !finalCode.includes("return") &&
          !finalCode.includes("export default")
        ) {
          // Try to find a defined component name
          const match =
            finalCode.match(/function\s+(\w+)/) ||
            finalCode.match(/const\s+(\w+)\s*=/);
          if (match && match[1]) {
            finalCode += `\nreturn ${match[1]};`;
          }
        ...
        if (typeof ResultComponent !== "function") {
          throw new Error(
            "The generated code did not return a valid React component.",
          );
        }
      ...
      >
        <div className="bg-card border border-red-500/20 rounded-2xl p-6 text-center">
          ...
          <p className="text-muted-foreground text-xs mb-4">
            {error || renderError || "Unknown error"}
          </p>
          {canRetry && (
            <button
              onClick={onRetry}
              className="px-5 py-2 bg-[#d97757] text-foreground rounded-lg text-sm font-semibold hover:brightness-110 transition-all flex items-center gap-2 mx-auto"
            >
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] bg-secondary/50 text-foreground/40 px-1.5 py-0.5 rounded border border-border font-mono">
                  TSX
                </span>
              </div>
              {description && (
        ...
        >
          {transpiling ? (
            ...
            <div className="p-8 text-center">
              ...
              <pre className="text-foreground/40 text-[10px] bg-card/40 p-4 rounded-xl mt-4 overflow-x-auto text-left border border-border">
                {renderError}
              </pre>
              {canRetry && (
                <button
                  onClick={onRetry}
                  className="mt-6 px-4 py-2 bg-secondary/50 text-foreground rounded-lg text-xs font-semibold hover:bg-secondary transition-all"
                >
                  Re-generate
                </button>
              )}
