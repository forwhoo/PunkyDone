...
export const HourlyChart: React.FC<HourlyChartProps> = ({ history }) => {
  ...
  const hourlyData = useMemo(() => {
    const hours = Array(24)
      .fill(0)
      .map((_, i) => ({ hour: i, count: 0, items: [] as any[] }));
    ...
    return hours.map((data) => {
      ...
      data.items.forEach((play) => {
        ...
        if (filter === "Artist") {
          ...
          label = key;
          image = play.artist_image || play.cover;
          // fallback
        ...
        ref[key] = {
          label,
          image,
          subLabel: filter === "Song" ? play.artist_name : "",
        };
      });
      ...
      <div className="h-48 flex items-end gap-1 md:gap-2">
        {hourlyData.map((d, i) => (
          ...
          >
            ...
            <div
              className="w-full bg-[#2C2C2E] rounded-t-sm relative overflow-hidden group-hover:bg-[#3C3C3E] transition-colors"
              style={{ height: `${(d.count / maxPlays) * 100}%` }}
            >
              {/* Fill Effect */}
              <motion.div
                initial={{ height: 0 }}
