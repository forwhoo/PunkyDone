...
export const SongList = ({ songs }: { songs: Song[] }) => {
  return (
    <Card className="bg-[#1C1C1E] border-none shadow-none h-[380px] overflow-hidden">
        <CardHeader className="flex flex-row items-center justify-between pb-4 pl-0 pr-2 pt-0">
             <CardTitle className="text-white text-xl font-bold px-5">Top Songs</CardTitle>
        ...
        <CardContent className="flex flex-col gap-0 px-0 overflow-y-auto no-scrollbar h-full pb-4">
            {songs.map((song, index) => (
                <div key={song.id} className="flex items-center px-5 py-3 hover:bg-[#2C2C2E] transition-colors duration-200 cursor-default group border-b border-[#2C2C2E]/50 last:border-0">
                    <div className="w-6 text-left text-[#8E8E93] text-sm font-semibold group-hover:text-white transition-colors">
                        {index + 1}
                    </div>
                    ...
                    <div className="flex-1 min-w-0 pr-4">
                        ...
                        <p className="text-[13px] text-[#8E8E93] truncate mt-0.5">{song.artist}</p>
                    </div>

                    <div className="text-right">
                        <span className="text-[13px] font-medium text-[#8E8E93] group-hover:text-white transition-colors block">
