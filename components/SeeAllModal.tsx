...
export const SeeAllModal: React.FC<SeeAllModalProps> = ({ isOpen, onClose, title, items, type }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-4 lg:p-6 w-full h-full">
            ...
            {/* Modal Content */}
            <div className="relative bg-[#1C1C1E] w-full max-w-5xl h-full md:h-[90vh] md:rounded-3xl lg:rounded-[40px] overflow-hidden shadow-2xl flex flex-col border border-white/10 animate-in slide-in-from-bottom-10 fade-in duration-500">
                
                {/* Header: Title + Sorting + Close */}
                <div className="absolute top-0 left-0 right-0 z-50 px-4 md:px-6 py-4 md:py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center bg-gradient-to-b from-[#1C1C1E] via-[#1C1C1E]/95 to-transparent pb-8 md:pb-12">
                     <div className="mb-3 sm:mb-0">
                        <h2 className="text-2xl md:text-3xl font-bold text-white tracking-tight">{title}</h2>
                        <p className="text-[#8E8E93] text-xs md:text-sm mt-1">{items.length} items ranked</p>
                     </div>
                     ...
                     <div className="flex items-center gap-2 w-full sm:w-auto">
                        ...
                        <button 
                            onClick={onClose}
                            className="bg-[#3A3A3C] hover:bg-[#48484A] text-white rounded-full p-2 transition-all hover:scale-105 active:scale-95 border border-white/10 flex-shrink-0"
                        >
                ...
                <div className="flex-1 overflow-y-auto px-3 md:px-6 lg:px-8 pt-24 sm:pt-28 md:pt-32 pb-8 md:pb-12 scrollbar-thin scrollbar-thumb-[#333] scrollbar-track-transparent">
                    { type === 'artist' ? (
                        /* CIRCULAR ARTIST GRID LAYOUT */
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-y-8 md:gap-y-12 gap-x-4 md:gap-x-6 p-2 md:p-4">
                            {sortedItems.map((item, index) => (
                                <div key={index} className="group relative flex flex-col items-center">
                                    <div className="relative w-28 h-28 sm:w-32 sm:h-32 md:w-36 md:h-36 rounded-full shadow-lg transition-transform duration-300 group-hover:scale-105 group-hover:shadow-2xl z-10 cursor-pointer border border-white/5 group-hover:border-white/20">
                                        {/* Artist Image */}
                                        <img 
                                        ...
                                        {/* Name Inside Image */}
                                        <div className="absolute inset-x-0 bottom-0 pb-2 md:pb-3 px-2 md:px-3 text-center">
                                            <h3 className="text-[11px] md:text-[12px] font-bold text-white drop-shadow-md truncate">{item.name}</h3>
                                        </div>
                                        ...
                                        <div className="absolute inset-0 rounded-full bg-black/60 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center text-center p-2">
                                            <span className="text-lg md:text-xl font-bold text-white mb-0">{item.totalListens || item.listens || 0}</span>
                                            <span className="text-[8px] md:text-[9px] uppercase tracking-widest text-white/70 mb-1">Plays</span>
                                            <span className="text-[10px] md:text-xs font-bold text-[#FA2D48] bg-white/10 px-2 py-0.5 rounded-full">{item.timeStr || '0m'}</span>
                                        </div>
                                    </div>
                                </div>
                            ))}
