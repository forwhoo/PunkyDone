    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-0 md:p-6 w-full h-full">
            ...
            <div className="relative bg-[#1C1C1E] w-full max-w-5xl h-full md:h-[85vh] md:rounded-[40px] overflow-hidden shadow-2xl flex flex-col border border-white/10 animate-in slide-in-from-bottom-10 fade-in duration-500">
                ...
                {/* Header Actions Only (No Banner) */}
                <div className="absolute top-6 right-6 z-50">
                    <button 
                        onClick={onClose}
                        className="bg-black/40 hover:bg-black/60 backdrop-blur-md text-white rounded-full p-2 transition-all hover:scale-105 active:scale-95 border border-white/10"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                {/* Scrollable Layout */}
                <div className="flex-1 overflow-y-auto px-4 md:px-8 py-12 scrollbar-thin scrollbar-thumb-[#333] scrollbar-track-transparent">
                    { type === 'artist' ? (
                            ...
                            {items.map((item, index) => (
                                <div key={index} className="group relative cursor-pointer">
                                    ...
                                    <div className="relative w-full aspect-square rounded-xl overflow-hidden bg-[#1C1C1E] z-20 shadow-xl border border-white/10 group-hover:-translate-y-1 transition-transform duration-300">
                                        ...
                                        {/* Hover Overlay Stats */}
                                        <div className="absolute inset-0 bg-black/60 backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center text-center p-3">
