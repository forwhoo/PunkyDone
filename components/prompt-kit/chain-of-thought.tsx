...
export function ChainOfThought({
  ...
  return (
    <div
      className={cn("flex flex-col gap-2 my-4 w-full max-w-2xl", className)}
      {...props}
    >
      {children}
    </div>
    ...
    >
      {React.Children.map(children, (child) => {
        if (
          ...
          child.type === ChainOfThoughtTrigger
        ) {
        ...
        if (
          React.isValidElement(child) &&
          child.type === ChainOfThoughtContent
        ) {
...
export function ChainOfThoughtItem({
  ...
  return (
    <div
      className={cn("text-muted-foreground/80 leading-relaxed", className)}
      {...props}
    >
      {children}
    </div>
