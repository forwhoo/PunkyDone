...
export const HistoryTimeline: React.FC<HistoryTimelineProps> = ({ history }) => {
    ...
    const getColor = (count: number, inYear: boolean) => {
        ...
        if (intensity < 0.25) return 'bg-[#d97757]/30';
        if (intensity < 0.50) return 'bg-[#d97757]/60';
        if (intensity < 0.75) return 'bg-[#d97757]/80';
        return 'bg-[#d97757] shadow-[0_0_8px_rgba(250,45,72,0.4)]';
    };
        ...
        <div className="mb-20 animate-in fade-in duration-700 select-none">
                            ...
                            {week.map((day, dIndex) => (
                                <div
                                    ...
                                    className={`w-3 h-3 md:w-3.5 md:h-3.5 rounded-[3px] transition-all duration-300 relative group ${
                                        selectedDate === day.date ? 'ring-2 ring-white z-20 scale-125 bg-[#d97757] shadow-[0_0_10px_#FA2D48]' : 'hover:scale-125 hover:z-10'
                        ...
                        <div className="p-6 relative">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 rounded-full bg-[#d97757]/20 flex items-center justify-center text-[#d97757]">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[#b0aea5] text-xs mt-1 font-medium">Daily Mix</p>
                                    </div>
                                </div>
                            ...
                            {selectedDayItems.length === 0 ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                    {selectedDayItems.map((item: any, i: number) => (
                                        <div key={i} className="flex items-center gap-4 p-3 rounded-xl bg-[#faf9f5]/40 hover:bg-[#e8e6dc]/50 transition-colors group border border-transparent hover:border-[#e8e6dc] cursor-default">
                                            <div className="relative w-12 h-12 flex-shrink-0 group-hover:scale-105 transition-transform duration-300">
                                                ...
                                                />
                                                <div className="absolute inset-0 bg-[#faf9f5]/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-md">
                                                </div>
                                            </div>
                                            <div className="min-w-0 flex-1 flex flex-col justify-center">
                                                ...
                                                <p className="text-[#b0aea5] text-[13px] truncate">{item.artist_name}</p>
                                            </div>
            ...
            {!selectedDate && (
                <div className="mt-4 p-4 rounded-xl border border-dashed border-[#e8e6dc] text-center">
                    <p className="text-[#b0aea5] text-xs">Select a square above to reveal your detailed listening history for that day.</p>
                </div>
            )}
        </div>
    );
