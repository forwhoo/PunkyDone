...
export const HistoryTimeline: React.FC<HistoryTimelineProps> = ({
  ...
  const getColor = (count: number, inYear: boolean) => {
    if (!inYear)
      return "bg-transparent border-none pointer-events-none opacity-0";
    // Hide days outside 2026
    if (count === 0) return "bg-[#2C2C2E]/50";
    const intensity = count / maxCount;
    if (intensity < 0.25) return "bg-[#d97757]/30";
    if (intensity < 0.5) return "bg-[#d97757]/60";
                ...
                <div
                  ...
                  className={`w-3 h-3 md:w-3.5 md:h-3.5 rounded-[3px] transition-all duration-300 relative group ${
                    selectedDate === day.date
                      ? "ring-2 ring-white z-20 scale-125 bg-[#d97757] shadow-[0_0_10px_#FA2D48]"
