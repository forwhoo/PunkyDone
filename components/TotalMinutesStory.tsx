...
const MAX_NODES = 25;
const NODE_SIZE = 80;
const NODE_FLY_STAGGER_MS = 140;
// Slightly longer for better rhythm
const NODE_JOURNEY_MS = 600;
// Longer journey for more dramatic effect
const NODE_CONSUME_MS = 250;
const HOLD_DURATION_MS = 2500;
const OPENING_DURATION_MS = 2000;
// Duration for firework opening
...
export const TotalMinutesStory: React.FC<TotalMinutesStoryProps> = ({
  ...
  const covers = useMemo(
    () => albumCovers.slice(0, nodeCount),
    [albumCovers, nodeCount],
  );
  ...
  return (
    ...
    >
      ...
      <style>{` @keyframes gridPulse { 0%, 100% { opacity: 0.5; } 50% { opacity: 0.8; } } `}</style>{" "}
        ...
        >
          ...
          <motion.div
            ...
            style={{
              ...
              borderRadius: "50%",
              background:
                "radial-gradient(circle, rgba(255,255,255,1) 0%, rgba(200,200,255,0.8) 40%, transparent 70%)",
            }}
            initial={{ scale: 0, opacity: 1 }}
            animate={{ scale: [0, 30, 50], opacity: [1, 0.6, 0] }}
          ...
          {[0, 0.2, 0.4].map((delay, i) => (
            <motion.div
              ...
              animate={{ scale: [0, 15], opacity: [0.8, 0] }}
            ...
            <motion.div
              ...
              animate={
                flyTriggered
                  ...
                  : { x: node.x, y: node.y, scale: 1, opacity: 1, rotate: 0 }
              }
              transition={
                flyTriggered
                  ? {
